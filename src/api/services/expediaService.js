...
async function scrapeUntilValid({
  expedia_listing_page_url,
  start_date,
  end_date,
  check_in_date,
  ...
  while (true) {
    const ret = await scrapeExpediaPage(
      expedia_listing_page_url,
      start_date,
      end_date,
      check_in_date,
...
async function scrapeExpedia(req) {
  ...
  for (let i = 0; i < body.length; i++) {
    let ranges = getFridayRanges(body[0].start_date, body[0].end_date);
    console.log(ranges);
    for (let j = 0; j < ranges.length; j++) {
      const promise = scrapeUntilValid({
        expedia_listing_page_url: body[i].expedia_listing_page_url,
        check_in_date: ranges[j][0],
        check_out_date: ranges[j][1],
        start_date: body[0].start_date,
...
async function scrapeExpediaPage(
  url = "",
  start_date = "",
  end_date = "",
  check_in_date = "",
